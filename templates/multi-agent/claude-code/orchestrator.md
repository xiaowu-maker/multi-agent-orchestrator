# 编排器行为规范（完整版）— 🅰 Claude Code 版

> **适用平台：Claude Code（Anthropic CLI）**
> - 子Agent 工具：Task / Subagent（一次性、隔离上下文）
> - ❌ 无"给已结束的 subagent 发消息续对话"的机制（subagent 是瞬时的，结束即失联）
> - 并行上限：`workflow.max_parallel`（默认3，按 Claude Code 并发能力调整）
> - 触发方式：`CLAUDE.md` 复制到项目根目录
> - 模板安装位置：`~/.claude/templates/multi-agent/`（用 install.sh / install.bat）
> - ⚠️ 本版与 Hermes 版 / dsh 版不通用，勿混用。其他版本见 `hermes/orchestrator.md`、`dsh/orchestrator.md`
...
### 规则3：状态恢复（Claude Code 无续对话，文件恢复是唯一途径）
子Agent（Task/subagent）是**一次性**的——不存在"给已结束的 subagent 发消息"的机制（Claude Code 的 SendMessage 只能让运行中的 subagent 向主会话汇报，不能让主会话复活一个已结束的 subagent）。
...
2. 没有 → 询问用户选预设（standard / with-review / minimal / ai-competition / 自定义）→ 从 `~/.claude/templates/multi-agent/presets/` 复制对应 yaml 写入 roles.yaml + workflow.yaml
...
- `task_strategy: parallel` → 无依赖任务分组，并行创建子Agent（同时最多 `workflow.max_parallel` 个，默认3，值见 workflow.yaml）
...
## 创建子Agent的标准格式（Claude Code：Task/subagent 工具调用）
...
- 模型名永远来自 roles.yaml，不要硬编码（Claude Code 可用 sonnet / haiku / opus 等）
